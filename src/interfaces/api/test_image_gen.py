...
IMAGE_MODEL = os.getenv("IMAGE_MODEL", "image-01")
TRUST_ENV = os.getenv("SOPHNET_TRUST_ENV", "").lower() in {"1", "true", "yes"}
API_URL = os.getenv("IMAGE_API_URL", "https://api.minimaxi.com/v1/image_generation")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "data" in result and isinstance(result["data"], dict):
                ...
                # MiniMax 返回格式: data.image_urls[0]
                if "image_urls" in data_obj and isinstance(data_obj["image_urls"], list):
                    if data_obj["image_urls"]:
                        image_url = data_obj["image_urls"][0]
