...
class GlobalSpecGenerator:
    ...
    async def generate_spec(
        ...
        # 新增: 估算分镜数
        shot_count_info = self._estimate_shot_count(duration, analysis.text_analysis)
        ...
        global_spec = GlobalSpec(
            ...
            estimated_shot_count=shot_count_info["estimated"],
            shot_count_range=shot_count_info["range"]
        )
    ...
    def _estimate_shot_count(
        ...
        基于以下因素:
        - duration: 视频时长(秒) - 主要因素
        - text_analysis.scenes: 场景数
        - narrative_structure: 叙事复杂度
        ...
        Returns:
            Dict包含estimated和range
        """
        # 基准计算: 每6秒1个镜头
        base_shot_count = max(duration // 6, 3)  # 至少3个镜头
        
        # 根据叙事复杂度调整
        multiplier = 1.0
        if text_analysis:
            # 检查叙事结构复杂度
            narrative = getattr(text_analysis, 'narrative_structure', 'linear')
            if narrative in ['non-linear', 'complex', 'multi-thread']:
                multiplier = 1.3  # 复杂叙事增加30%
            elif narrative in ['simple', 'minimal']:
                multiplier = 0.8  # 简单叙事减少20%
            
            # 如果有明确的场景数,也作为参考
            scenes = getattr(text_analysis, 'scenes', [])
            if scenes and len(scenes) > 0:
                # 每个场景至少1个镜头,最多3个镜头
                scene_based_estimate = len(scenes) * 2  # 平均每场景2镜头
                # 如果基于场景的估算更合理,使用加权平均
                base_shot_count = int((base_shot_count + scene_based_estimate) / 2)
        
        # 计算最终估算值
        estimated = int(base_shot_count * multiplier)
        
        # 计算容差范围 (±20%)
        tolerance = 0.2
        min_shots = max(int(estimated * (1 - tolerance)), 3)
        max_shots = int(estimated * (1 + tolerance))
        ...
        logger.debug(
            f"Estimated shot count: {estimated} (range: {min_shots}-{max_shots})",
            extra={
                "duration": duration,
                "base_count": base_shot_count,
