...
class CLIPDetector:
    ...
    def compute_similarity(self, image1, image2):
        ...
        Args:
            image1: numpy array 或 PIL Image
            image2: numpy array 或 PIL Image
            
        Returns:
            float: 相似度 (0-1)
        """
        if self.model is None:
            logger.warning("CLIP model not loaded, using fallback")
            # 返回基于像素的简单相似度
            return self._compute_pixel_similarity(image1, image2)
    ...
    def _compute_pixel_similarity(self, image1, image2):
        """基于像素的简单相似度（fallback）"""
        try:
            ...
            logger.error(f"Pixel similarity failed: {e}")
            return 0.5
