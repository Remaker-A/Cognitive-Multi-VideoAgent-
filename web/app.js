...
const API_BASE_URL = 'http://localhost:8000/api'; // TODO: 替换为实际 API 地址
...
async function apiCall(endpoint, data) {
    try {
        ...
        if (error.name === 'AbortError' || error.message.includes('Failed to fetch')) {
            setApiStatus(false);
            throw new Error('无法连接后端服务，请检查后端是否启动以及 API_BASE_URL 配置');
        }
            ...
            <div class="shot-card" data-shot="${i + 1}">
                <div class="shot-header">
                    ...
                    <div class="shot-header-info">
                        ...
                        <div class="shot-meta">
                            <span class="meta-item"><span class="meta-icon">⏱️</span> ${s.duration || 0}秒</span>
                ...
                <div class="shot-body">
                    <div class="shot-section">
                        ...
                        <div class="section-content">${s.description || '无描述'}</div>
                    </div>
                    ${s.visual_elements ? `
                    <div class="shot-section">
...
async function generateImages() {
        ...
        grid.innerHTML += `
            <div class="image-card" id="img-${i}">
                ...
                <div class="image-info">
                    ...
                    <div class="image-status">生成中...</div>
                </div>
            </div>
        `;
    }
    ...
    for (let i = 1; i <= totalShots; i++) {
        ...
        try {
            ...
            const shotInfo = shots[i - 1] || null; // 数组索引从 0 开始

            const response = await apiCall(`${API_BASE_URL}/generate-image`, {
                project_id: projectId,
                shot: i,
                shot_info: shotInfo  // 传递分镜详细信息
            });
            ...
            card.querySelector('.image-status').textContent = response.success ? '已完成' : '使用占位图';
...
async function generateVideo() {
    ...
    try {
        ...
        // 降级到示例视频
        document.getElementById('video-status').style.display = 'none';
        document.getElementById('final-video').src = 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4';
        document.getElementById('video-result').style.display = 'block';
    }
}
...
function setApiStatus(ok) {
    ...
    if (!statusEl) return;
    statusEl.textContent = ok ? '后端已连接' : '后端未连接';
}
