...
async def generate_script(data: ScriptInput):
    ...
    system_prompt = """你是一个专业的视频剧本作家。
你的任务是根据用户需求和分析结果，创作一个完整的视频剧本。
剧本应该包含：
1. 每个镜头的详细描述
2. 镜头时长
3. 视觉元素
...
- 建议镜头数：{analysis.get('shots', 8)}
- 总时长：{analysis.get('duration', 30)} 秒

请根据以上需求分析，创作一个专业的视频剧本。
剧本应该结构清晰，易于理解和执行，并且完全符合用户的原始需求。"""
